...
namespace Task01
{
    class Program
    {
        static void Main()
        {
            ...
            Console.WriteLine("------------------------");
            TestCollections<int, string> testCollection = new TestCollections<int, string>(5, "Oleksandr");
            testCollection.PrintList1();

            int keyToCheck = 3;
            bool containsKey = testCollection.ContainsKey(keyToCheck);
            Console.WriteLine($"Чи містить myDict1 ключ {keyToCheck}? {containsKey}");
            keyToCheck = 10;
            containsKey = testCollection.ContainsKey(keyToCheck);
            Console.WriteLine($"Чи містить myDict1 ключ {keyToCheck}? {containsKey}");

            IEnumerable<string> values = testCollection.GetValues();
            Console.WriteLine("Елементи в myDict2:");
        ...
        {
            private List<TKEY> myList1;
            private List<string> myList2;
            private Dictionary<TKEY, TValue> myDict1;
            private Dictionary<string, TValue> myDict2;

            public TestCollections(int numberOfElements, string TempName)
            {
                myList1 = new List<TKEY>();
                myList2 = new List<string>();
                myDict1 = new Dictionary<TKEY, TValue>();
                myDict2 = new Dictionary<string, TValue>();
                ...
                for (int i = 0; i < numberOfElements; i++)
                {
                    ...
                    {
                        value = GenerateValue(TempName.Substring(0, TempName.Length - i));
                    }
                    else
                    {
                        value = GenerateValue(TempName);
                    }
                    string stringKey = key.ToString();
                    myList1.Add(key);
                    myList2.Add(Convert.ToString(i));
                    myDict1.Add(key, value);
                    myDict2.Add(Convert.ToString(i), value);
                }
            }

            private static TValue GenerateValue(string parameter)
            {
                return (TValue)(object)parameter;
            }

            private static TKEY GenerateKey(int parameter)
            {
                return (TKEY)(object)parameter;
            }

            public void PrintList1()
            {
                Console.WriteLine("Елементи в myList1:");
            ...
            {
                return myDict1.ContainsKey(key);
            }

            public IEnumerable<TValue> GetValues()
            {
                return myDict2.Values;
            }
        }
